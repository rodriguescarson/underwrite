...
- **Second opinion** — a different model (Claude Sonnet 4.5 via OpenRouter, or an open-source model on Featherless) reviews the *specific* legs with the same read-only MCP tools and must agree; the desk records the more conservative P(profit) and every veto. `UNDERWRITE_SECOND_MODEL`, `UNDERWRITE_COMMITTEE=review|independent`.
- **Gate** — deterministic Python. Re-quotes every leg through alpaca-py, recomputes max loss itself, enforces 1% of equity per structure, DTE 7–45, bid/ask ≤ 15%, open interest ≥ 100, at most one structure per underlying, four total. Sizes from **measured calibration**, not from confidence. One structure per underlying, six open at most.
