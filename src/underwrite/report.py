...
def build() -> tuple[str, str]:
    ...
    md = [f"# Underwrite — desk report", f"_Generated {datetime.now(timezone.utc).isoformat(timespec='minutes')} from the ledger and nothing else. Action channel: Alpaca MCP server (`uvx alpaca-mcp-server`). Audit channel: Alpaca CLI {last.get('cli_version', '')}. Strategist: Gemini via Google ADK, read-only MCP tools._", "",
          "## The idea in one line", "The strategist proposes, a deterministic gate re-prices and sizes, the executor acts through Alpaca's MCP server, an independent auditor re-checks every claim through the Alpaca CLI, and position size is earned by measured calibration rather than granted by confidence.", "",
          "## How to verify this report", "Every order id below appears twice: once in the executor's claim (what the MCP server returned) and once in the CLI audit (what the account actually holds). Every gate decision lists its reasons. Every proposal lists the MCP tool calls it made. The raw ledger is at `ledger.json` next to this page; the code is at github.com/rodriguescarson/underwrite.", ""]
    ...
    text = "\n".join(md)
    banner = ("<div style='display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin:1rem 0 1.5rem'>"
              + "".join(f"<div style='border:1px solid #ddd;border-radius:8px;padding:.6rem .8rem;background:#fff'><div style='font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#666'>{k}</div><div style='font-size:22px;font-weight:600'>{v}</div></div>" for k, v in [("claims", state.n_claims), ("CLI-confirmed", f"{state.claim_accuracy if state.claim_accuracy is not None else '—'}"), ("silent failures", state.silent_failures), ("resolved", f"{state.n_resolved}/10"), ("risk / structure", f"{state.risk_frac:.2%}")])
              + "</div>"
              + "<div style='display:flex;gap:10px;align-items:center;font-size:13px;color:#333;margin-bottom:1rem'><span style='border:1px solid #111;border-radius:6px;padding:.3rem .6rem'>Strategist · Gemini/ADK</span>→<span style='border:1px solid #111;border-radius:6px;padding:.3rem .6rem'>Gate · deterministic</span>→<span style='border:1px solid #111;border-radius:6px;padding:.3rem .6rem;background:#111;color:#fff'>Executor · Alpaca MCP</span>→<span style='border:1px solid #111;border-radius:6px;padding:.3rem .6rem;background:#111;color:#fff'>Auditor · Alpaca CLI</span>→<span style='border:1px solid #111;border-radius:6px;padding:.3rem .6rem'>Calibration → size</span></div>")
    body = _md_to_html(text)
    body = body.replace("</h1>", "</h1>" + banner, 1)
    page = "<!doctype html><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>Underwrite — desk report</title><style>body{font:15px/1.5 system-ui,sans-serif;max-width:1100px;margin:2rem auto;padding:0 1rem;color:#111;background:#f7f6f3}table{border-collapse:collapse;width:100%;font-size:13px;margin:.5rem 0 1.5rem;background:#fff}th,td{border:1px solid #ddd;padding:.35rem .5rem;text-align:left;vertical-align:top}th{background:#f3f3f3}h1{margin-bottom:0;letter-spacing:-.01em}h2{margin-top:2rem;border-bottom:1px solid #ddd;padding-bottom:.25rem}code{background:#eee;padding:0 .25rem}p{max-width:80ch}</style>" + body
    return text, page
